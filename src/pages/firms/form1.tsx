...
const Form1 = () => {
  ...
  const applicantDetailsChange = (e: any) => {
    ...
    if (AddName == "maskedAadhar") {
      ...
      setApplicantDetails(updatedApplicant)
      syncApplicantToPartners(updatedApplicant);
      console.log(updatedApplicant, "updatedApplicant")
    } else {
      const updatedApplicant = { ...TempDetails, [AddName]: AddValue };
      setApplicantDetails(updatedApplicant)
      syncApplicantToPartners(updatedApplicant);
    }
  }
  ...
  const syncApplicantToPartners = (updatedApplicant: IApplicantDetailsForm1Model) => {
    const fullName = updatedApplicant.name?.trim() || `${updatedApplicant.firstName} ${updatedApplicant.surName}`.trim()
    const applicantAsPartner: ISelectedPartnerDetailsModel = {
      maskedAadhar: updatedApplicant.maskedAadhar,
      aadharNumber: updatedApplicant.aadharNumber,
      applicantName: updatedApplicant.firstName,
      surName: updatedApplicant.surName,
      relationType: updatedApplicant.relationType,
      relation: updatedApplicant.relation,
      gender: updatedApplicant.gender,
      age: updatedApplicant.age,
      role: updatedApplicant.role,
      doorNo: updatedApplicant.doorNo,
      street: updatedApplicant.street,
      district: updatedApplicant.district,
      mandal: updatedApplicant.mandal,
      villageOrCity: updatedApplicant.villageOrCity,
      pinCode: updatedApplicant.pinCode,
      ...
      share: "",
      joiningDate: "",
      otpCode: updatedApplicant.otpCode,
      otpStatus: updatedApplicant.otpStatus,
      otp: updatedApplicant.otp,
      OTPResponse: updatedApplicant.OTPResponse,
      KYCResponse: updatedApplicant.KYCResponse,
      address: updatedApplicant.address || ""
    }

    let clone = [...partnerDetails];
    if (clone.length > 0) {
      clone[0] = applicantAsPartner;
    } else {
      clone.push(applicantAsPartner);
    }
    setPartnerDetails(clone);
  }

  const contactDetailsChange = (e: any) => {
    const { name, value } = e.target;
    setContactDetails((prev: any) => ({ ...prev, [name]: value }));
  ...
  const addPartnerFields = () => {
    if (!partnerDetails || partnerDetails.length === 0) {
      ...
      return ShowMessagePopup(false, "Please add applicant details before adding partner", "");
    }
    if (
      partnerDetails.length > 0 &&
      (!partnerDetails[0].aadharNumber ||
        partnerDetails[0].aadharNumber.trim() === "")
    ) {
      return ShowMessagePopup(false, "Applicant details are incomplete. Please fill applicant details first", "");
    }
    ...
    if (
      ...
      if (
        partnerDetails.length > 0 &&
        editingIndex !== 0 &&
        (!partnerDetails[0].share || Number(partnerDetails[0].share) <= 0)
      ) {
        return ShowMessagePopup(false, "Please enter share for Applicant (Applicant is a partner and must have share)", "");
      }
      let share = 0
  ...
  const handleSubmit = async (e: any) => {
    ...
    newData.append("others", file.others)
  ...
  return (
    <>
      <Head>
        <title>E-Registration of Firm</title>
            ...
            {(firmData.status == "Incomplete" || isResubmission == "true") && (
              <div className="maindivContainer">
                ...
                <Col lg={12} md={12} xs={12}>
                  ...
                  <div>
                    <div className="pageTitleLeft d-flex justify-content-between align-items-center page-title mb-2">
                      <h5>Applicant is one of the partner.</h5>
                    </div>
                  </div>
                </Col>
              </Row>
              ...
              >
                  <div className="regofAppBg mb-3">
                    ...
                    <TableText label="Landline Phone No" required={false}  />
                    <TableInputText
                      disabled={false}
                      type="number"
                  ...
                  <div className="regofAppBg regofAppBG mb-3">
                    <Row>
                  ...
                  <div className="regofAppBg mb-3">
                    ...
                    <TableInputText disabled={false} type='text' placeholder='Enter Applicant SirName' required={false} name={'surName'} value={SelectedPartnerDetails.surName} onChange={partnerDetailsChange} />
                        ...
                        <Col lg={3} md={3} xs={12} className="mb-3"><TableText label="Gender" required={false} /><TableInputText
                          disabled={true}
                          type="text"
                          placeholder="Enter Gender"
                          required={false}
                  ...
                  <div className="appDocList mb-4">
                    <Card>
                      <Card.Header>Upload Firm Related Documents-(All Uploaded Documents should be in PDF format only upto 3MB )</Card.Header>
                      <Card.Body>
                        <Card.Text>
                        ...
                        <Form.Group controlId="formFile">
                          ...
                          />
                              ...
                              <Col lg={3} md={4} xs={12}>
                                <div className="firmFile">
                                  <Form.Group controlId="formFile">
                                    <TableText label="Others (If Any)" required={false} />
                                    <Form.Control
                                      type="file"
                                      id="others"
                                      name="others"
                  ...
                  <div className="firmSubmitSec mt-3 mb-3">
                    <Row >
